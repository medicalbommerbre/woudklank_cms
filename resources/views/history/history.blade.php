    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded float-left" 
                    onclick="window.location.href='{{ url("/history/create") }}'">
                Maak een nieuw geschiedenis item aan
            </button>
        </div>
    </x-slot>
                ...
                <tbody>
                    @foreach ($history as $history)
                        <tr class="bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700">
                            <td class="py-5 px-4 border-b text-white">{{ $history->priority }}</td>
                            <td class="py-5 px-4 border-b text-white">{{ $history->title }}</td>
                            <td class="py-5 px-4 border-b text-white">{{ $history->description }}</td>
                            <td class="py-5 px-4 border-b text-white">
                                <a href="{{ route('history.edit', ['history' => $history->id]) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Bewerken</a>
                            </td>
                            <td class="py-5 px-4 border-b text-white">
